...
export class MQTTPubSub implements PubSubEngine {
  ...
  constructor(options: PubSubMQTTOptions = {}) {
    ...
    // this.parseMessageWithEncoding = options.parseMessageWithEncoding; //base64
  }

  public publish(triggerName: string, payload: any): Promise<void> {
    return new Promise((resolve, reject) => {
      this.publishOptionsResolver(triggerName, payload).then(
        (publishOptions) => {
          const message = Buffer.from(JSON.parse(payload), "base64");
  ...
  private onMessage(topic: string, message: Buffer) {
    ...
    const messageString = message.toString("base64");
    let parsedMessage;
    try {
